...
# 🚀 Day-2: Dockerize the Node.js Starter Project
...
touch Dockerfile
```
### 4. Paste the below content into the docker file -- This Dockerfile uses a multi-stage build where Node.js is used to build the application and Nginx is used to serve the static build output.
``` bash
...
``` bash
docker run -dp 3000:80 username/<image/repo-to-be-pushed>:tag
```
